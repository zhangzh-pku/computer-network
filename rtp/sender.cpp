...
int sender(char *receiver_ip, char *receiver_port, int window_size, char *message)
{
    ...
    while (seq < len) // data数据的传输
    {
        ...
        for (int i = 0; (i < window_size) && (seq + i < len); i++)
        {
            ...
            rtp->checksum = 0;
            memcpy(rtp + sizeof(rtp_header_t), file_data[i], rtp->length);
        ...
        int tem = 1;
        int max_seq = seq;
        while (total < window_size && tem != 0) //如果已经有windows_size个文件，那就不需要再等
        {
            ...
            else
            {
                ...
                {
                    if (!ack[rtp->seq_num - 1]) //针对error code3 防止ack重传
                    {
                        ack[rtp->seq_num - 1] = true;
                        total++;
                    }
                    if (max_seq < (int)rtp->seq_num)
                    {
                        max_seq = rtp->seq_num;
                    }
                    std::cout << "the " << rtp->seq_num - 1 << " packet ack is received\n";
                }
                else
                {
                    std::cout << "the " << rtp->seq_num - 1 << " packet has a wrong ack\n";
                }
            }
        }
        seq = max_seq;
        std::cout << "the least seqnum is " << max_seq - 1 << "\n";
    }
